...
jQuery(document).ready(function(){var stickyNavTop=jQuery('.header-wrapper').height();var stickyNav=function(){var scrollTop=jQuery(window).scrollTop();if(scrollTop>stickyNavTop){jQuery('.header-wrapper').addClass('stick')}else{jQuery('.header-wrapper').removeClass('stick')}};stickyNav();jQuery(window).scroll(function(){stickyNav()});jQuery('#site-navigation').onePageNav({currentClass:'current-one-page-item',changeHash:!1,scrollSpeed:1500,scrollThreshold:0.5,filter:'',easing:'swing',begin:function(){},end:function(){},scrollChange:function(jQuerycurrentListItem){}});jQuery('.scrollup').click(function(){jQuery("html, body").animate({scrollTop:0},2000);return !1});jQuery('.map-btn').click(function(){jQuery('#map iframe').slideToggle('slow');jQuery('.map-btn i').toggleClass('fa-angle-double-up');jQuery('.map-btn i').toggleClass('fa-angle-double-down')});jQuery('.search-icon').click(function(){jQuery('.search-box').toggleClass('active')});jQuery('.search-box .close').click(function(){jQuery('.search-box').removeClass('active')});jQuery(window).on('load',function(){var width=Math.max(window.innerWidth,document.documentElement.clientWidth);if(width&&width<=768){jQuery('.menu-toggle,#site-navigation a').click(function(){jQuery('#site-navigation .menu-primary-container,#site-navigation div.menu').slideToggle()})}});jQuery('#site-navigation .menu-item-has-children').append('<span class="sub-toggle"> <i class="fa fa-angle-right"></i> </span>');jQuery('#site-navigation .sub-toggle').click(function(){jQuery(this).parent('.menu-item-has-children').children('ul.sub-menu').first().slideToggle('1000');jQuery(this).children('.fa-angle-right').first().toggleClass('fa-angle-down')})});
...
setTimeout(function(){var percent;$.each(['alt','longdesc','aria-describedby'],function(i,val){var attr=$(settings.el).attr(val)||$(settings.el).attr('data-'+val);if(attr){photo.setAttribute(val,attr)}});var isRetina=!1;if(settings.get('retinaImage')&&window.devicePixelRatio>1){isRetina=!0;if(isRetina){if(photo.width<jQuery(window).width()&&photo.height<jQuery(window).height()){photo.height=photo.height/window.devicePixelRatio;photo.width=photo.width/window.devicePixelRatio}else{if(jQuery(window).width()<jQuery(window).height()){var k=(photo.height/ window.devicePixelRatio) / (photo.width/window.devicePixelRatio);photo.height=k*jQuery(window).width()-70;photo.width=jQuery(window).width()-70}else{if(photo.height>photo.width){var k=(photo.width/ window.devicePixelRatio) / (photo.height/window.devicePixelRatio);photo.height=jQuery(window).height()-70;photo.width=k*jQuery(window).height()-70}else{var k=(photo.height/ window.devicePixelRatio) / (photo.width/window.devicePixelRatio);photo.height=k*jQuery(window).width()-70;photo.width=jQuery(window).width()-70}}}}}
...
publicMethod.next=function(){if(!active&&$related[1]&&(settings.get('loop')||$related[index+1])){index=getIndex(1);launch($related[index])}};publicMethod.prev=function(){if(!active&&$related[1]&&(settings.get('loop')||index)){index=getIndex(-1);launch($related[index])}};publicMethod.close=function(){if(open&&!closing){closing=!0;open=!1;trigger(event_cleanup);settings.get('onCleanup');$window.unbind('.'+prefix);$overlay.fadeTo(settings.get('fadeOut')||0,0);$box.stop().fadeTo(settings.get('fadeOut')||0,0,function(){$box.hide();$overlay.hide();trigger(event_purge);$loaded.remove();setTimeout(function(){closing=!1;settings.get('onClosed')},1)})}
if(jQuery('#g-main-slider_'+lightbox_obj.galleryID)[0]){var api=jQuery.data(jQuery('#g-main-slider_'+lightbox_obj.galleryID)[0],'liquidSlider');setTimeout(function(){api.startAutoSlide()},lightbox_obj.liquidSliderInterval)}};publicMethod.remove=function(){if(!$box){return}
